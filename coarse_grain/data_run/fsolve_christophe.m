function [x,FVAL,EXITFLAG,OUTPUT,JACOB] = fsolve_christophe(funfcn,x,options,varargin)
%FSOLVE solves systems of nonlinear equations of several variables.
%
%   This is a modified FSOLVE, essentially it strips out all
%   customizability afforded by the "options" that are passed in, the
%   result is approximately 2x faster than fsolve averaged across many runs
%
...
%   $Revision: 1.1.6.21 $  $Date: 2011/04/11 16:19:26 $
...
%pcf hack, we always use levenberg
algorithmflag = 3;
...
options.FinDiffType = 'forward'; %options.FinDiffType = optimget(options,'FinDiffType',defaultopt,'fast'); 
options.GradObj = 'on'; %options.GradObj = optimget(options,'Jacobian',defaultopt,'fast');
options.GradConstr = 'off';
DerivativeCheck = 0; %DerivativeCheck = strcmp(optimget(options,'DerivativeCheck',defaultopt,'fast'),'on');
...
options.DiffMinChange = 0; %options.DiffMinChange = optimget(options,'DiffMinChange',defaultopt,'fast');
options.DiffMaxChange = Inf; %options.DiffMaxChange = optimget(options,'DiffMaxChange',defaultopt,'fast');
options = validateFinDiffRelStep(sizes.nVar,options,defaultopt);

% Create structure of flags for finitedifferences
finDiffFlags.fwdFinDiff = 1; %finDiffFlags.fwdFinDiff = strcmpi(options.FinDiffType,'forward'); % Check for forward fin-diff
...
sqrtTolFun = .01; %sqrtTolFun = sqrt(optimget(options,'TolFun',defaultopt,'fast'));
...
%    OUTPUT.message = createExitMsg(msqrtTolFunsgData{:});
end
